...
def Nx_Grads(u_value,d_0,device=device):
...
class Model_a(nn.Module):
    def __init__(self,model):
        ...
        self.model = model
        self.coeff = nn.Parameter(torch.tensor([0.5]),requires_grad=True)

    def forward(self, x,t):
        pre_u=self.model(x,t)
        return pre_u
...
model_Unet=Model_a(model_Unet).to(device)
par = list(model_trans.parameters())+list(model_Unet.parameters())
opt = torch.optim.Adam(params=par, lr=lr)
scheduler = optim.lr_scheduler.StepLR(opt, step_size=40000, gamma=0.2)

mse = nn.MSELoss(reduction='none').to(device)
dict1=diffusion.diff_dict
pbar = tqdm(total=epochs // 100, desc="Epochs")

for epoch in range(epochs):
    coeff=model_Unet.coeff
    ...
    pre_f0 = Nx_Grads(pre_u0*model_Unet.coeff, d_0=2 / pre_u0.shape[-1])
    ...
    if loss_pde>1:
        loss = loss_vdm1  * 100 +loss_pde**0.5
    else:
        loss = loss_vdm1*10 +loss_pde*10
    opt.zero_grad()
    loss.backward()
    opt.step()
    scheduler.step()
    ...
    if (epoch+1) % 2000 == 0:
        ...
        torch.save(models_para, "{}/PIGD_inverse_{}.pth".format(save_dir, epoch+1))
